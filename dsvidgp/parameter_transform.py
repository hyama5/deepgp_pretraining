"""Transform parameters to contrain value range."""
...
class Logistic:
    ...
    def forward(self, x):
        """Convert real numpy number to (0, 1) using sigmoid."""
        return 1.0 / (1.0 + np.exp(-x))

    def forward_tensor(self, x):
        """Convert real number tensor to (0, 1)."""
        return torch.sigmoid(x)

    def backward(self, y):
        """Convert (0, 1) numpy number to real."""
...
class Positive:
    ...
    def __init__(self, lower=1e-6):
        ...
        Parameters.
            lower: float, minimum value of output
        """
        self._lower = lower
